...
export class LiveMetricsEngine {
    ...
    constructor(startingBalanceSol = 10, private readonly clock: () => number = Date.now) {
        this.startingBalanceSol = startingBalanceSol;
    }
    ingest(event: NormalizedMarketEvent): void {
        if (!event.mint)
            return;
        const now = Date.parse(event.receivedAt) || this.clock();
        const existing = this.tokens.get(event.mint);
        const token: TokenLiveState = existing ??
            {
                mint: event.mint,
                firstSeenAt: now,
                lastSeenAt: now,
        ...
        if (event.kind === "trade" && event.txType?.startsWith("inferred_")) {
            ...
            const sol = lamports !== undefined ? lamports / 1000000000 : 0;
            const vSol = event.virtualSolReserves;
            const vToken = event.virtualTokenReserves;
            const priceRatio = vSol !== undefined &&
                vToken !== undefined &&
                ...
                ? vSol / vToken
                : undefined;
            token.trades.push({ ts: now, side, sol, priceRatio, post: event.txType.startsWith("inferred_swap_") });
            this.pushEvent(`${side.toUpperCase()} ${token.symbol ?? shortMint(token.mint)} ${sol.toFixed(4)} SOL`);
        }
        const cutoff = this.clock() - 5 * 60000;
    ...
    rowForMint(mint: string): TokenDashboardRow | undefined {
        ...
        return token ? this.toRow(token, this.clock()) : undefined;
    }
    snapshot(limit = 80, bots?: PaperBotSummary[], extraEvents: string[] = []): DashboardSnapshot {
    ...
    private toRow(token: TokenLiveState, now: number): TokenDashboardRow {
        const w10 = this.window(token, now, 10000);
        const w30 = this.window(token, now, 30000);
        const w60 = this.window(token, now, 60000);
        ...
        return {
            ...
            w30,
            w60,
            hybrid: this.assessHybrid(ageSeconds, token.migratedAt !== undefined, token.postMigrationLive === true, w10, w30, w60),
            migration: this.assessMigration(token.migratedAt !== undefined, token.postMigrationLive === true),
        };
    }
    private window(token: TokenLiveState, now: number, windowMs: number): WindowMetrics {
        const trades = token.trades.filter((trade) => trade.ts >= now - windowMs && trade.ts <= now && (token.migratedAt === undefined ? !trade.post : trade.post));
        if (trades.length === 0)
            return emptyWindow();
        let buys = 0;
        let sells = 0;
        ...
        for (const trade of trades) {
            if (trade.sol < config.paperMinSampleSol)
                continue;
            if (trade.side === "buy") {
                buys += 1;
                buySol += trade.sol;
        ...
        return {
            ...
            buySellVolumeRatio: sellSol > 0 ? buySol / sellSol : buySol > 0 ? 99 : null,
            priceChangePct,
        };
    }
    private assessHybrid(ageSeconds: number, migrated: boolean, postMigrationLive: boolean, w10: WindowMetrics, w30: WindowMetrics, w60: WindowMetrics): StrategyAssessment {
        if (migrated) {
            return {
    ...
    private assessMigration(migrated: boolean, postMigrationLive: boolean): StrategyAssessment {
        return migrated
            ? {
                status: postMigrationLive ? "PUMPSWAP LIVE" : "WAIT PUMPSWAP DATA",
                reasons: postMigrationLive
    ...
    private pushEvent(message: string): void {
        ...
        if (this.recentEvents.length > 40)
            this.recentEvents.length = 40;
    }
    private pruneTokens(): void {
        const cutoff = this.clock() - 20 * 60000;
        for (const [mint, token] of this.tokens) {
            if (token.lastSeenAt < cutoff)
...
function hasConstructiveRecentFlow(w10: WindowMetrics): boolean {
    return (w10.buySellVolumeRatio !== null &&
