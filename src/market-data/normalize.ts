import type { EventKind, NormalizedMarketEvent, UnknownRecord, } from "../types/market.js";
...
function inferKind(raw: UnknownRecord): EventKind {
    ...
    if (txType === "buy" || txType === "sell")
        return "trade";
    if (raw.message !== undefined || raw.error !== undefined || raw.errors !== undefined)
        return "system";
    return "unknown";
}
export function normalizeMarketEvent(input: unknown): NormalizedMarketEvent {
    ...
    return {
        ...
        traderPublicKey: asString(raw.traderPublicKey) ??
            asString(raw.trader) ??
            asString(raw.user),
        ...
        marketCapSol: asNumber(raw.marketCapSol) ??
            asNumber(raw.marketCap) ??
            asNumber(raw.solMarketCap),
        virtualSolReserves: asNumber(raw.vSolInBondingCurve) ??
            asNumber(raw.virtualSolReserves),
        virtualTokenReserves: asNumber(raw.vTokensInBondingCurve) ??
            asNumber(raw.virtualTokenReserves),
        bondingCurveKey: asString(raw.bondingCurveKey) ??
